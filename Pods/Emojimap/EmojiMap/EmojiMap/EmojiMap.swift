...
open class EmojiMap {
    ...
    public func emojiDataBase(_ json: String = "") -> NSDictionary {
        
        // Language
        var pre = ""
        if(json == ""){
            let fullLanguage = Locale.preferredLanguages[0]
            pre = String(fullLanguage.prefix(2))
            pre = "-" + pre
        }else{
            pre = "-" + json
        }
        print(Bundle(for: EmojiMap.self))
        // Search for file in main bundle
        if let file = Bundle.main.path(forResource: "emojis" + pre, ofType: "json"),
